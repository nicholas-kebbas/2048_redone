'use strict';
angular.module('Game', ['Grid', 'ngCookies']).service('GameManager', function(
  $q, $timeout, GridService, $cookieStore) {
    ...
    var f = function() {
      ...
      positions.x.forEach(function(x) {
        positions.y.forEach(function(y) {
          ...
          if (tile) {
            var cell = GridService.calculateNextPosition(tile,
                key),
              next = cell.next;
            ...
            if (!GridService.samePositions(originalPosition,
              cell.newPosition)) {
              hasMoved = true;
            }
          }
